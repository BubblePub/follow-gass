...
Mark any cluster with `hasOfficial === true` with "🏛️ 含本人/竞选官方口径": include the
official-site link in the 来源 list labelled as 官方 (not as a neutral outlet), and in the
对比 make clear what is the person's own framing vs. how media reported it (see
`compare-sources.md`). Never present the official line as independent confirmation.
...
(`type: "media"`), or a future engagement announced in an official-site item
(`type: "official"`, e.g. "premier meeting le 30 mai") that has no `eventDate` field of
its own. Include one ONLY if a title/snippet states a specific activity with a date
and/or place and the nature of the event (e.g. "Attal sera à Lille mardi"). For an
official-site engagement, tag it "据官方" rather than "据媒体报道". **Only include activities of the
