...
# 2. 无需 npm install —— scripts/ 零外部依赖,仅需 Node.js ≥ 18
node --version   # 确认环境即可
```

然后在 Claude Code 里输入 **`/gass`**。首次会引导你选「跟谁 + 投递方式」(输出固定中文),之后直接出当天简报。

### 方式二:命令行直接运行

不用任何 AI agent,也可以直接拿到 feed 数据:

```bash
git clone https://github.com/BubblePub/follow-gass
cd follow-gass/scripts

# 输出今日 feed JSON(含聚类所需字段)
node prepare-digest.js

# 通过邮件或 stdout 投递(需先配置 ~/.follow-gass/config.json)
node deliver.js --file /tmp/digest.txt
```

`prepare-digest.js` 从公开的中央 `feed.json` 读取数据,纯本地运行,无需网络权限,可接入任何支持管道的工具链。

### 方式三:其他 AI 工具(OpenClaw 等)

将仓库克隆到对应 skill 目录,SKILL.md 会自动被识别。工具启动时会检测平台并切换投递方式(OpenClaw 走持久频道,其他走 stdout/邮件)。

**零配置、零 API key、零依赖**——内容来自公开的中央 feed,不用 fork、不用改任何文件。
