...
5. 把仓库 clone 成 skill:
   ```bash
   git clone https://github.com/<你的用户名>/follow-attal-sejourne ~/.claude/skills/follow-attal-sejourne
   cd ~/.claude/skills/follow-attal-sejourne/scripts && npm install
   ```
6. 在 agent 里说「setup follow-attal-sejourne」或 `/digest`,按引导选择跟谁、语言、投递方式。

## 自定义

所有 prompt 都是纯文本(`prompts/*.md`),对话式即可改:「概括短一点」「对比再深一点」
「多关注 2027 初选」。改动会复制到 `~/.follow-attal-sejourne/prompts/` 持久保存,不被中央更新覆盖。
